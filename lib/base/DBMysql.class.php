...
class DBMysql
{
	...
	function __construct($host, $user, $password, $database, $port=3306, $forceNew = false)
	{
		$this->m_host = $host;
		$this->m_user = $user;
		...
		$this->m_errstr = NULL;
		//初始化链接
		$this->m_instance = mysql_connect(($this->m_host.':'.$this->m_port), $this->m_user, $this->m_password, $forceNew);
		if (!($this->m_instance))
		{
	...
	public function SetDatabase($database)
	{
		...
		{
			throw new OssException('Can\'t use '.$this->m_database.' : ' . mysql_error($this->m_instance).".\n");
		}
	}
	...
	public function ExecQuery($stmt,&$resultset)
	{
		...
		if(!$result)
		{
			throw new OssException('Invalid query:['.$stmt.'] Error:'.mysql_error($this->m_instance).".\n");
		}
	...
	public function ExecTrans($statements)
	{
		...
		for($i=0; $i<count($statements); ++$i)
		{
			...
			{
				$this->Rollback();
				throw new OssException('Mysql query:['.$statements[$i].'] Error:'.mysql_error($this->m_instance).".\n");
			}
			if($sql_type != SQLT_INSERT  && mysql_affected_rows($this->m_instance) == 0 )
			{
				...
				throw new OssException('Mysql query:['.$statements[$i].'] not affected any rows, Error:'.mysql_error($this->m_instance).".\n");
			}
		}
		$this->Commit();
		return 0;
	...
	public function Begin()
	{
		...
		{
			throw new OssException('Invalid query: [start transaction;] Error:'.mysql_error($this->m_instance).".\n");
		}
		return 0 ;
	}
	...
	public function Commit()
	{
		...
		{
			throw new OssException('Invalid query: [commit;] Error:'.mysql_error($this->m_instance).".\n");
		}
		return 0;
	}
	...
	public function Rollback()
	{
		...
		{
			throw new OssException('Invalid query: [rollback;] Error:'.mysql_error($this->m_instance).".\n");
		}
	 	return 0;
	}
