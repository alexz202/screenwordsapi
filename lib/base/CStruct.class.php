...
class TypeArray implements Iterator, Countable, ArrayAccess {
    ...
    public function __construct($type, $len = 0, $value = null) {
        if(!class_exists($type)) throw "error create type array：invalid type!";
        $this->_type = $type;
    ...
    public function create_element() { $elem = new $this->_type; return $elem; }
    public function offsetExists($offset) { return isset($this->_values[$offset]); }
    ...
    public function offsetGet($offset) { return isset($this->_values[$offset]) ? $this->_values[$offset] : null; }
    public function offsetUnset($offset) { unset($this->_values[$offset]); }
...
class LVArray extends StructBase implements Iterator, Countable, ArrayAccess {
    ...
    public function offsetGet($offset) { return isset($this->values[$offset]) ? $this->values[$offset] : null; }
    public function offsetUnset($offset) { unset($this->values[$offset]); }
    public function rewind() { reset($this->values->GetValue()); }
    public function current() { return current($this->values->GetValue()); }
    public function key() { return key($this->values->GetValue()); }
    public function next() { return next($this->values->GetValue()); }
    public function valid() { return ( $this->current() !== false ); }
    public function count() { return count($this->values->GetValue()); }
}
